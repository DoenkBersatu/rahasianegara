...
function epi_block_lang_injectors() {
	if (!empty($_GET['language'])) {
		$getlength = strlen($_GET['language']);
		$getlangstr = $_GET['language'];
		if ($getlength > 6) {
			die('Lang injection blocked');
		}
		$banpaths = array('/etc','../');
		foreach ($banpaths as $key=>$banpath) {
			if (strpos($getlangstr,$banpath)) {
