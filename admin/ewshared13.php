...
function ew_LocaleConv() {
	...
	$cleaned = strval(str_replace("\0", "", $localefile));
	$cleaned = strval(str_replace("../", "",$cleaned));
	if (!file_exists($cleaned)) {

		// Locale file not found, fall back to English ("en") locale
		$cleaned = EW_LOCALE_FOLDER . "en.json";
	}
	$locale = json_decode(file_get_contents($cleaned), TRUE);
