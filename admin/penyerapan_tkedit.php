...
$penyerapan_tk_edit = NULL; // Initialize page object first

class cpenyerapan_tk_edit extends cpenyerapan_tk {
	...
	// Page object name
	var $PageObjName = 'penyerapan_tk_edit';
	...
	function Page_Main() {
		...
		// Check if valid key
		if ($this->id_penyerapan_tk->CurrentValue == "") {
			$this->Page_Terminate("penyerapan_tklist.php"); // Invalid key, return to list
		}
	...
	function LoadFormValues() {
		...
		// Load from form
		global $objForm;
		if (!$this->id_penyerapan_tk->FldIsDetailKey)
			$this->id_penyerapan_tk->setFormValue($objForm->GetValue("x_id_penyerapan_tk"));
	...
	function RestoreFormValues() {
		global $objForm;
		$this->LoadRow();
		$this->id_penyerapan_tk->CurrentValue = $this->id_penyerapan_tk->FormValue;
	...
	function EditRow() {
		...
		if ($rs->EOF) {
			...
			// jumlah_penyerapan
			$this->jumlah_penyerapan->SetDbValueDef($rsnew, $this->jumlah_penyerapan->CurrentValue, 0, $this->jumlah_penyerapan->ReadOnly);
...
// Create page object
if (!isset($penyerapan_tk_edit)) $penyerapan_tk_edit = new cpenyerapan_tk_edit();

// Page init
$penyerapan_tk_edit->Page_Init();

// Page main
$penyerapan_tk_edit->Page_Main();

// Global Page Rendering event (in userfn*.php)
Page_Rendering();

// Page Rendering event
$penyerapan_tk_edit->Page_Render();
?>
...
var CurrentForm = fpenyerapan_tkedit = new ew_Form("fpenyerapan_tkedit", "edit");

// Validate form
fpenyerapan_tkedit.Validate = function() {
...
fpenyerapan_tkedit.Form_CustomValidate = 
...
fpenyerapan_tkedit.ValidateRequired = true;
<?php } else { ?>
fpenyerapan_tkedit.ValidateRequired = false; 
<?php } ?>

// Dynamic selection lists
fpenyerapan_tkedit.Lists["x_id_triwulan"] = {"LinkField":"x_ID_triwulan","Ajax":true,"AutoFill":false,"DisplayFields":["x_triwulan","x_tahun","",""],"ParentFields":[],"ChildFields":[],"FilterFields":[],"Options":[],"Template":"","LinkTable":"triwulan"};
fpenyerapan_tkedit.Lists["x_id_sektor"] = {"LinkField":"x_id_sektor","Ajax":true,"AutoFill":false,"DisplayFields":["x_sektor","","",""],"ParentFields":[],"ChildFields":["x_id_subsektor"],"FilterFields":[],"Options":[],"Template":"","LinkTable":"sektor"};
fpenyerapan_tkedit.Lists["x_id_subsektor"] = {"LinkField":"x_id_subsektor","Ajax":true,"AutoFill":false,"DisplayFields":["x_subsektor","","",""],"ParentFields":["x_id_sektor"],"ChildFields":[],"FilterFields":["x_id_sektor"],"Options":[],"Template":"","LinkTable":"subsektor"};
...
<?php if (!$penyerapan_tk_edit->IsModal) { ?>
<?php } ?>
<?php $penyerapan_tk_edit->ShowPageHeader(); ?>
<?php
$penyerapan_tk_edit->ShowMessage();
?>
<form name="fpenyerapan_tkedit" id="fpenyerapan_tkedit" class="<?php echo $penyerapan_tk_edit->FormClassName ?>" action="<?php echo ew_CurrentPage() ?>" method="post">
<?php if ($penyerapan_tk_edit->CheckToken) { ?>
<input type="hidden" name="<?php echo EW_TOKEN_NAME ?>" value="<?php echo $penyerapan_tk_edit->Token ?>">
<?php } ?>
...
<?php if ($penyerapan_tk->id_penyerapan_tk->Visible) { // id_penyerapan_tk ?>
	<div id="r_id_penyerapan_tk" class="form-group">
		<label id="elh_penyerapan_tk_id_penyerapan_tk" class="col-sm-2 control-label ewLabel"><?php echo $penyerapan_tk->id_penyerapan_tk->FldCaption() ?></label>
		<div class="col-sm-10"><div<?php echo $penyerapan_tk->id_penyerapan_tk->CellAttributes() ?>>
<span id="el_penyerapan_tk_id_penyerapan_tk">
<span<?php echo $penyerapan_tk->id_penyerapan_tk->ViewAttributes() ?>>
<p class="form-control-static"><?php echo $penyerapan_tk->id_penyerapan_tk->EditValue ?></p></span>
</span>
<input type="hidden" data-table="penyerapan_tk" data-field="x_id_penyerapan_tk" name="x_id_penyerapan_tk" id="x_id_penyerapan_tk" value="<?php echo ew_HtmlEncode($penyerapan_tk->id_penyerapan_tk->CurrentValue) ?>">
<?php echo $penyerapan_tk->id_penyerapan_tk->CustomMsg ?></div></div>
	</div>
<?php } ?>
...
<button class="btn btn-default ewButton" name="btnCancel" id="btnCancel" type="button" data-href="<?php echo $penyerapan_tk_edit->getReturnUrl() ?>"><?php echo $Language->Phrase("CancelBtn") ?></button>
	</div>
</div>
<?php } ?>
</form>
<script type="text/javascript">
fpenyerapan_tkedit.Init();
</script>
<?php
$penyerapan_tk_edit->ShowPageFooter();
if (EW_DEBUG_ENABLED)
	echo ew_DebugMsg();
?>
<script type="text/javascript">
...
<?php
$penyerapan_tk_edit->Page_Terminate();
?>
