...
//
// Table class for penyerapan_tk
//
class cpenyerapan_tk extends cTable {
	var $id_penyerapan_tk;
	var $id_triwulan;
	var $id_sektor;
	var $id_subsektor;
	var $jumlah_penyerapan;
	...
	function __construct() {
		...
		$this->TableVar = 'penyerapan_tk';
		$this->TableName = 'penyerapan_tk';
		$this->TableType = 'TABLE';

		// Update Table
		$this->UpdateTable = "`penyerapan_tk`";
		$this->DBID = 'DB';
		$this->ExportAll = TRUE;
		...
		// id_penyerapan_tk
		$this->id_penyerapan_tk = new cField('penyerapan_tk', 'penyerapan_tk', 'x_id_penyerapan_tk', 'id_penyerapan_tk', '`id_penyerapan_tk`', '`id_penyerapan_tk`', 3, -1, FALSE, '`id_penyerapan_tk`', FALSE, FALSE, FALSE, 'FORMATTED TEXT', 'NO');
		$this->id_penyerapan_tk->Sortable = TRUE; // Allow sort
		$this->id_penyerapan_tk->FldDefaultErrMsg = $Language->Phrase("IncorrectInteger");
		$this->fields['id_penyerapan_tk'] = &$this->id_penyerapan_tk;

		// id_triwulan
		$this->id_triwulan = new cField('penyerapan_tk', 'penyerapan_tk', 'x_id_triwulan', 'id_triwulan', '`id_triwulan`', '`id_triwulan`', 3, -1, FALSE, '`id_triwulan`', FALSE, FALSE, FALSE, 'FORMATTED TEXT', 'SELECT');
		...
		// id_sektor
		$this->id_sektor = new cField('penyerapan_tk', 'penyerapan_tk', 'x_id_sektor', 'id_sektor', '`id_sektor`', '`id_sektor`', 3, -1, FALSE, '`id_sektor`', FALSE, FALSE, FALSE, 'FORMATTED TEXT', 'SELECT');
		...
		// id_subsektor
		$this->id_subsektor = new cField('penyerapan_tk', 'penyerapan_tk', 'x_id_subsektor', 'id_subsektor', '`id_subsektor`', '`id_subsektor`', 3, -1, FALSE, '`id_subsektor`', FALSE, FALSE, FALSE, 'FORMATTED TEXT', 'SELECT');
		...
		// jumlah_penyerapan
		$this->jumlah_penyerapan = new cField('penyerapan_tk', 'penyerapan_tk', 'x_jumlah_penyerapan', 'jumlah_penyerapan', '`jumlah_penyerapan`', '`jumlah_penyerapan`', 20, -1, FALSE, '`jumlah_penyerapan`', FALSE, FALSE, FALSE, 'FORMATTED TEXT', 'TEXT');
		$this->jumlah_penyerapan->Sortable = TRUE; // Allow sort
		$this->jumlah_penyerapan->FldDefaultErrMsg = $Language->Phrase("IncorrectInteger");
		$this->fields['jumlah_penyerapan'] = &$this->jumlah_penyerapan;
	}
	...
	function getSqlFrom() { // From
		return ($this->_SqlFrom <> "") ? $this->_SqlFrom : "`penyerapan_tk`";
	}
	...
	function Insert(&$rs) {
		...
		if ($bInsert) {
			...
			$this->id_penyerapan_tk->setDbValue($conn->Insert_ID());
			$rs['id_penyerapan_tk'] = $this->id_penyerapan_tk->DbValue;
		}
		return $bInsert;
	}
	...
	function DeleteSQL(&$rs, $where = "", $curfilter = TRUE) {
		...
		if (is_array($where))
			$where = $this->ArrayToFilter($where);
		if ($rs) {
			if (array_key_exists('id_penyerapan_tk', $rs))
				ew_AddFilter($where, ew_QuotedName('id_penyerapan_tk', $this->DBID) . '=' . ew_QuotedValue($rs['id_penyerapan_tk'], $this->id_penyerapan_tk->FldDataType, $this->DBID));
		}
		$filter = ($curfilter) ? $this->CurrentFilter : "";
		ew_AddFilter($filter, $where);
		if ($filter <> "")
	...
	function SqlKeyFilter() {
		return "`id_penyerapan_tk` = @id_penyerapan_tk@";
	}

	// Key filter
	function KeyFilter() {
		$sKeyFilter = $this->SqlKeyFilter();
		if (!is_numeric($this->id_penyerapan_tk->CurrentValue))
			$sKeyFilter = "0=1"; // Invalid key
		$sKeyFilter = str_replace("@id_penyerapan_tk@", ew_AdjustSql($this->id_penyerapan_tk->CurrentValue, $this->DBID), $sKeyFilter); // Replace key value
		return $sKeyFilter;
	}
	...
	function GetViewUrl($parm = "") {
		if ($parm <> "")
			$url = $this->KeyUrl("penyerapan_tkview.php", $this->UrlParm($parm));
		else
			$url = $this->KeyUrl("penyerapan_tkview.php", $this->UrlParm(EW_TABLE_SHOW_DETAIL . "="));
	...
	function GetAddUrl($parm = "") {
		if ($parm <> "")
			$url = "penyerapan_tkadd.php?" . $this->UrlParm($parm);
		else
			$url = "penyerapan_tkadd.php";
		return $this->AddMasterUrl($url);
	}

	// Edit URL
	function GetEditUrl($parm = "") {
		$url = $this->KeyUrl("penyerapan_tkedit.php", $this->UrlParm($parm));
		return $this->AddMasterUrl($url);
	}
	...
	function GetCopyUrl($parm = "") {
		$url = $this->KeyUrl("penyerapan_tkadd.php", $this->UrlParm($parm));
		return $this->AddMasterUrl($url);
	}
	...
	function GetDeleteUrl() {
		return $this->KeyUrl("penyerapan_tkdelete.php", $this->UrlParm());
	}
	...
	function KeyToJson() {
		$json = "";
		$json .= "id_penyerapan_tk:" . ew_VarToJson($this->id_penyerapan_tk->CurrentValue, "number", "'");
		return "{" . $json . "}";
	}
	...
	function KeyUrl($url, $parm = "") {
		$sUrl = $url . "?";
		if ($parm <> "") $sUrl .= $parm . "&";
		if (!is_null($this->id_penyerapan_tk->CurrentValue)) {
			$sUrl .= "id_penyerapan_tk=" . urlencode($this->id_penyerapan_tk->CurrentValue);
		} else {
	...
	function GetRecordKeys() {
		...
		if (isset($_POST["key_m"])) {
			...
			if ($isPost && isset($_POST["id_penyerapan_tk"]))
				$arKeys[] = ew_StripSlashes($_POST["id_penyerapan_tk"]);
			elseif (isset($_GET["id_penyerapan_tk"]))
				$arKeys[] = ew_StripSlashes($_GET["id_penyerapan_tk"]);
			else
	...
	function GetKeyFilter() {
		...
		foreach ($arKeys as $key) {
			...
			$this->id_penyerapan_tk->CurrentValue = $key;
   ...
   // Common render codes
	...
	function RenderEditRow() {
		...
		// id_penyerapan_tk
		$this->id_penyerapan_tk->EditAttrs["class"] = "form-control";
		$this->id_penyerapan_tk->EditCustomAttributes = "";
		$this->id_penyerapan_tk->EditValue = $this->id_penyerapan_tk->CurrentValue;
		$this->id_penyerapan_tk->ViewCustomAttributes = "";
		...
		$this->jumlah_penyerapan->EditValue = $this->jumlah_penyerapan->CurrentValue;
		...
		if (!$Doc->ExportCustom) {
			...
			if ($Doc->Horizontal) { // Horizontal format, write header
				...
				if ($ExportPageType == "view") {
					if ($this->id_penyerapan_tk->Exportable) $Doc->ExportCaption($this->id_penyerapan_tk);
					...
					if ($this->jumlah_penyerapan->Exportable) $Doc->ExportCaption($this->jumlah_penyerapan);
				} else {
					if ($this->id_penyerapan_tk->Exportable) $Doc->ExportCaption($this->id_penyerapan_tk);
					if ($this->id_triwulan->Exportable) $Doc->ExportCaption($this->id_triwulan);
					if ($this->id_sektor->Exportable) $Doc->ExportCaption($this->id_sektor);
		...
		while (!$Recordset->EOF && $RecCnt < $StopRec) {
			...
			if (intval($RecCnt) >= intval($StartRec)) {
				...
				if (!$Doc->ExportCustom) {
					...
					if ($ExportPageType == "view") {
						if ($this->id_penyerapan_tk->Exportable) $Doc->ExportField($this->id_penyerapan_tk);
						...
						if ($this->jumlah_penyerapan->Exportable) $Doc->ExportField($this->jumlah_penyerapan);
					} else {
						if ($this->id_penyerapan_tk->Exportable) $Doc->ExportField($this->id_penyerapan_tk);
						if ($this->id_triwulan->Exportable) $Doc->ExportField($this->id_triwulan);
						if ($this->id_sektor->Exportable) $Doc->ExportField($this->id_sektor);
