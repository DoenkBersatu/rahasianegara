...
$realisasi_investasi_add = NULL; // Initialize page object first

class crealisasi_investasi_add extends crealisasi_investasi {
	...
	// Page object name
	var $PageObjName = 'realisasi_investasi_add';
	...
	function Page_Main() {
		...
		if (@$_POST["a_add"] <> "") {
			...
			if (@$_GET["id_realisasi_investasi"] != "") {
				...
				$this->setKey("id_realisasi_investasi", $this->id_realisasi_investasi->CurrentValue); // Set up key
			} else {
				$this->setKey("id_realisasi_investasi", ""); // Clear key
		...
		switch ($this->CurrentAction) {
			...
			case "A": // Add new record
				...
				if ($this->AddRow($this->OldRecordset)) { // Add successful
					...
					elseif (ew_GetPageName($sReturnUrl) == "realisasi_investasiview.php")
	...
	function LoadDefaultValues() {
		...
		$this->jumlah_investasi->CurrentValue = NULL;
		$this->jumlah_investasi->OldValue = $this->jumlah_investasi->CurrentValue;
	}
	...
	function AddRow($rsold = NULL) {
		...
		// jumlah_investasi
		$this->jumlah_investasi->SetDbValueDef($rsnew, $this->jumlah_investasi->CurrentValue, 0, FALSE);
...
// Create page object
if (!isset($realisasi_investasi_add)) $realisasi_investasi_add = new crealisasi_investasi_add();

// Page init
$realisasi_investasi_add->Page_Init();

// Page main
$realisasi_investasi_add->Page_Main();

// Global Page Rendering event (in userfn*.php)
Page_Rendering();

// Page Rendering event
$realisasi_investasi_add->Page_Render();
?>
...
var CurrentForm = frealisasi_investasiadd = new ew_Form("frealisasi_investasiadd", "add");

// Validate form
frealisasi_investasiadd.Validate = function() {
...
frealisasi_investasiadd.Form_CustomValidate = 
...
frealisasi_investasiadd.ValidateRequired = true;
<?php } else { ?>
frealisasi_investasiadd.ValidateRequired = false; 
<?php } ?>

// Dynamic selection lists
frealisasi_investasiadd.Lists["x_id_triwulan"] = {"LinkField":"x_ID_triwulan","Ajax":true,"AutoFill":false,"DisplayFields":["x_triwulan","x_tahun","",""],"ParentFields":[],"ChildFields":[],"FilterFields":[],"Options":[],"Template":"","LinkTable":"triwulan"};
frealisasi_investasiadd.Lists["x_id_sektor"] = {"LinkField":"x_id_sektor","Ajax":true,"AutoFill":false,"DisplayFields":["x_sektor","","",""],"ParentFields":[],"ChildFields":["x_id_subsektor"],"FilterFields":[],"Options":[],"Template":"","LinkTable":"sektor"};
frealisasi_investasiadd.Lists["x_id_subsektor"] = {"LinkField":"x_id_subsektor","Ajax":true,"AutoFill":false,"DisplayFields":["x_subsektor","","",""],"ParentFields":["x_id_sektor"],"ChildFields":[],"FilterFields":["x_id_sektor"],"Options":[],"Template":"","LinkTable":"subsektor"};
...
<?php if (!$realisasi_investasi_add->IsModal) { ?>
<?php } ?>
<?php $realisasi_investasi_add->ShowPageHeader(); ?>
<?php
$realisasi_investasi_add->ShowMessage();
?>
<form name="frealisasi_investasiadd" id="frealisasi_investasiadd" class="<?php echo $realisasi_investasi_add->FormClassName ?>" action="<?php echo ew_CurrentPage() ?>" method="post">
<?php if ($realisasi_investasi_add->CheckToken) { ?>
<input type="hidden" name="<?php echo EW_TOKEN_NAME ?>" value="<?php echo $realisasi_investasi_add->Token ?>">
<?php } ?>
...
<button class="btn btn-default ewButton" name="btnCancel" id="btnCancel" type="button" data-href="<?php echo $realisasi_investasi_add->getReturnUrl() ?>"><?php echo $Language->Phrase("CancelBtn") ?></button>
	</div>
</div>
<?php } ?>
</form>
<script type="text/javascript">
frealisasi_investasiadd.Init();
</script>
<?php
$realisasi_investasi_add->ShowPageFooter();
if (EW_DEBUG_ENABLED)
	echo ew_DebugMsg();
?>
<script type="text/javascript">
...
<?php
$realisasi_investasi_add->Page_Terminate();
?>
