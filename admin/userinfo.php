...
class cuser extends cTable {
	...
	function __construct() {
		...
		// userlevel
		$this->userlevel = new cField('user', 'user', 'x_userlevel', 'userlevel', '`userlevel`', '`userlevel`', 3, -1, FALSE, '`userlevel`', FALSE, FALSE, FALSE, 'FORMATTED TEXT', 'SELECT');
   ...
   // Common render codes
	...
	function UserIDFilter($userid) {
		$sUserIDFilter = '`id_user` = ' . ew_QuotedValue($userid, EW_DATATYPE_NUMBER, EW_USER_TABLE_DBID);
	...
	function AddUserIDFilter($sFilter) {
		...
		if (!$this->UserIDAllow($id) && !$Security->IsAdmin()) {
			...
			if ($sFilterWrk <> "")
				$sFilterWrk = '`id_user` IN (' . $sFilterWrk . ')';
		}
